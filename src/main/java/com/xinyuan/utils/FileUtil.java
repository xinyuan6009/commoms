...
public class FileUtil {
     ...
     * @throws IOException
     */
    public static File mergePartFilesNoThread(String dirPath, String partFileSuffix,
            int partFileSize, String mergeFileName) throws IOException {
        ArrayList<File> partFiles = FileUtil.getDirFiles(dirPath,
                partFileSuffix);
        ...
        randomAccessFile.close();
         for (int i = 0; i < partFiles.size(); i++) {
            mergeNoThread(i * partFileSize,mergeFileName, partFiles.get(i));
        }
         String megpath=dirPath+mergeFileName;
         File megfile=new File(megpath);
         return megfile;
    }
    /**
     * 根据文件名，比较文件
   ...
   public  static void mergeNoThread(long startPos, String mergeFileName, File partFile){
       RandomAccessFile rFile;
       try {
   ...
   public void writeFile() throws IOException, InterruptedException {
       ...
       long originFileSize = 1024 * 1024 * 5;// 30M
       int blockFileSize = 1024 * 1024 * 2;// 15M
